...
accuracy_u = np.linalg.norm((u_true - predicted_u))

predicted_E = nn_E.predict(x_c[:BATCH_SIZE])
accuracy_E = np.linalg.norm((E_true - predicted_E))

with open(model_dir + 'results.csv', mode='w') as results_file:
    fieldnames = ['accuracy_u', 'accuracy_E']
    writer = csv.DictWriter(results_file, fieldnames=fieldnames)

    writer.writeheader()
    writer.writerow({'accuracy_u': accuracy_u, 'accuracy_E': accuracy_E})